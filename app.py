...
def parse_atendimentos(file_bytes: bytes, filename: str = "") -> Tuple[pd.DataFrame, Optional[float]]:
...
def main():
    ...
    with st.sidebar:
        ...
        if token_ok:
            ...
            st.warning("Sem GITHUB_TOKEN/GITHUB_REPO em secrets. Vou salvar somente localmente (não persistente no Cloud).")
    ...
    with tab_conv:
        ...
        st.write("Cadastre uma vez. Nos próximos relatórios, o app reaproveita automaticamente.")
        ...
        with col2:
            st.caption("Chave do convênio remove códigos entre parênteses. Ex.: 'BRADESCO - DIRETO(1001)' vira 'BRADESCO - DIRETO'.")
    ...
    with tab_proc:
        ...
        if up is None:
            ...
            try:
                df, report_total = parse_atendimentos(up.getvalue(), filename=up.name)
            except Exception as e:
                st.error(f"Falha ao ler o arquivo: {e}")
                st.stop()

            st.caption(
                f"Linhas: {len(df):,} | Convênios: {df['ConvenioKey'].nunique():,} | Guias: {df['Nr. Guia'].nunique():,}"
            )
            ...
            if novos:
                st.warning(f"Encontrados {len(novos)} convênios ainda sem faturamento cadastrado.")
            ...
            if report_total is not None:
                st.caption(f"Total informado no relatório: {format_brl(report_total)}")
